...
@implementer(interfaces.ILDAPEntry, interfaces.IEditableLDAPEntry, interfaces.IConnectedLDAPEntry)
class LDAPEntryWithClient(entry.EditableLDAPEntry):
    ...
    def _canRemove(self, key, value):
        ...
        for rdn in self.dn.split()[0].split():
            if rdn.attributeType == key and rdn.value == value:
                raise CannotRemoveRDNError("%s, %s" % (key, value))
    ...
    def _canRemoveAll(self, key):
        ...
        self._checkState()
        import types
        assert not isinstance(self.dn, bytes)
        ...
        if self._state != 'ready':
            if self._state == 'deleted':
                ...
                raise ObjectInBadStateError(
                    "State is %s while expecting %s" % (repr(self._state), 'ready'))
    ...
    def keys(self):
        self._checkState()
        return list(super(LDAPEntryWithClient, self).keys())

    def items(self):
        self._checkState()
        return list(super(LDAPEntryWithClient, self).items())
    ...
    def undo(self):
        ...
        for k, vs in list(self._remoteData.items()):
    ...
    def setPasswordMaybe_Samba(self, newPasswd):
        ...
        """
        if not self.complete and 'objectClass' not in self:
    ...
    def _cbSetPassword(self, dl, names):
        ...
        for name, (ok, x) in zip(names, dl):
            if not ok:
                l.append((name, x))
        if l:
            raise PasswordSetAggregateError(str(l))
        return self
    ...
    def __repr__(self):
        x = {}
        for key in list(super(LDAPEntryWithClient, self).keys()):
            x[key] = self[key]
        keys = list(x.keys())
        keys.sort()
        a = []
